...
def confirmation():
    if status.get() == 1:
        ...
        c.execute('SELECT title, quantity FROM books')
        for i in c.fetchall():
            if i[0] == nameEntry.get():
                if i[1] > 0:
...
frame3 = LabelFrame(root, text="Check availability".center(19), padx=6, pady=6)
frame3.grid(row=7, column=0, padx=6, pady=6)

bookLabel = Label(frame3, text="Name: ", anchor=E)
bookLabel.grid(row=0, column=0)
bookEntry = Entry(frame3, width=30, borderwidth=3)
bookEntry.grid(row=0, column=1)

def bookCheck():
    c.execute('SELECT title, quantity FROM books')
    for i, j in c.fetchall():
        if bookEntry.get().lower() == i.lower():
            Label(frame3, text="There are " + str(j) + " copies available.").grid(row=2, column=0)
            break
        else:
            Label(frame3, text="This title is not available.").grid(row=2, column=0)

confirm1 = Button(frame3, text="Confirm", padx=3, pady=3, command=bookCheck)
confirm1.grid(row=1, column=1)
